...
import { area, bbox, buildPolygon, buildRelaxed, fitPolygon, rotate180, spanAt, squareCorners, widthGuide } from '../src/geometry.js';

const near = (a, b, tol = 1e-6) => assert.ok(Math.abs(a - b) < tol, `${a} !== ${b}`);

/**
 * These tests describe a piece by the edges as they land on the page — left,
 * top, right, base — which is also the order they are entered in: clockwise
 * from the bottom-left corner.
 */

test('3-4-5 triangle is right-angled and 6 units of area', () => {
  const { vertices } = buildPolygon([3, 4, 5]);
  near(area(vertices), 6);
  near(bbox(vertices).width, 5);
});

test('triangle inequality is rejected rather than drawn', () => {
  assert.match(buildPolygon([40, 50, 100]).error, /Triangle inequality/);
});

test('four equal edges give a square, not a rhombus', () => {
  const { vertices } = buildPolygon([20, 20, 20, 20]);
...
test('a symmetric trapezoid closes with the top parallel to the bottom', () => {
  ...
  const { vertices } = buildPolygon([side, 100, side, 120]);
  near(bbox(vertices).height, 40, 1e-9);
  near(area(vertices), ((120 + 100) / 2) * 40, 1e-6);
  near(vertices[1].y, vertices[2].y);
});

test('side 1 is the left-hand edge on the page and side 4 the base', () => {
  // Clockwise from the bottom-left corner, so the walk round the drawing is the
  // walk round the piece. Left 40, base 120: the left edge stands vertical at
  // x = 0 and the base runs 120 along the bottom.
  const { vertices } = buildPolygon([40, 100, Math.hypot(20, 40), 120]);
  assert.deepEqual([vertices[0].x, vertices[0].y], [0, 0]);
  near(vertices[1].x, 0);
  near(vertices[1].y, 40);
  near(vertices[3].x, 120);
});
...
test('edges that cannot close as a trapezoid are rejected', () => {
  assert.match(buildPolygon([10, 100, 10, 200]).error, /No trapezoid closes/);
});

test('parallel edges of equal length with unequal sides are rejected', () => {
  assert.match(buildPolygon([30, 50, 40, 50]).error, /cannot be parallel/);
});

test('an unmeasured piece is empty, not an error', () => {
  assert.equal(buildPolygon([0, 0, 0, 0]).empty, true);
});

test('a half turn preserves area and bounding box', () => {
  const { vertices } = buildPolygon([Math.hypot(10, 40), 100, Math.hypot(10, 40), 120]);
...
test('a piece whose sides run parallel is drawn square to the base, not dropped', () => {
  ...
  const { vertices, model, error } = buildRelaxed([200, Math.hypot(70, 50), 150, 70]);
...
test('a trapezoid that closes is still drawn as a trapezoid', () => {
  const side = Math.hypot(10, 40);
  assert.equal(buildRelaxed([side, 100, side, 120]).model, 'trapezoid');
});

test('the fit changes the one edge it has to, not all four', () => {
  const fitted = fitPolygon([159, 85, 40, 70]).edges;
  assert.deepEqual([fitted[0], fitted[2], fitted[3]], [159, 40, 70]);
  near(fitted[1], Math.round(Math.hypot(70, 119) * 10) / 10, 1e-9);
});

test('a fitted triangle closes and stays close to what was measured', () => {
  const { edges, vertices } = fitPolygon([40, 50, 100]);
  assert.ok(vertices);
  assert.ok(area(vertices) > 0);
  edges.forEach((e, i) => assert.ok(Math.abs(e - [40, 50, 100][i]) < 6, `edge ${i} moved too far`));
});

test('edges that cannot be measured at all have no fit', () => {
  assert.equal(fitPolygon([0, 0, 0, 0]), null);
  assert.equal(fitPolygon(['a', 2, 3]), null);
});
...
test('the guide reads the base back when the piece is square between the bars', () => {
  ...
  const edges = [200, Math.hypot(69, 40), 240, 69];
...
test('there is no width guide for a triangle or for edges that make no shape', () => {
  assert.equal(widthGuide(buildPolygon([3, 4, 5]).vertices, [3, 4, 5]), null);
  assert.equal(widthGuide(null, [171, 104, 241, 69]), null);
  assert.equal(widthGuide(buildRelaxed([171, 104, 241, 69]).vertices, [171, 0, 241, 69]), null);
});
...
test('square corners are found where the measurements can tell, and not where they lean', () => {
  ...
  assert.deepEqual(squareCorners(buildPolygon([3, 4, 5]).vertices), [1]);
  assert.deepEqual(squareCorners(buildPolygon([100, 100, 100, 100]).vertices), [0, 1, 2, 3]);
  // A right-angled trapezoid: square at both ends of the vertical side.
  assert.deepEqual(squareCorners(buildPolygon([40, 100, Math.hypot(20, 40), 120]).vertices), [0, 1]);
  // The same piece with that side read off the tape as 44.72 is 0.02° out, which
  // is the tape, not the piece — it keeps both marks.
  assert.deepEqual(squareCorners(buildPolygon([40, 100, 44.72, 120]).vertices), [0, 1]);
  // A symmetric trapezoid leans at every corner, and gets nothing.
  assert.deepEqual(squareCorners(buildPolygon([Math.hypot(10, 40), 100, Math.hypot(10, 40), 120]).vertices), []);
  // Where the line is: piece 6 of this roof is 0.4° off at the corner where its
  // 78 base meets the 88 side, which is the tape and not the piece, so it is
  // marked — and would not be on a tolerance tighter than the tape.
  assert.deepEqual(squareCorners(buildRelaxed([88, 118, 78]).vertices), [0]);
  assert.deepEqual(squareCorners(buildRelaxed([88, 118, 78]).vertices, 0.3), []);
});
